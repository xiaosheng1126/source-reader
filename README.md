...
- 普通 URL：静态网页、博客、官方文档；优先抽取 `article/main` 正文，并保留标题、描述、作者、发布时间、预览图、JSON-LD、正文区标题层级、链接、图片和小型表格等结构化 metadata。
...
- RSS/Atom：读取 feed 标题、条目、发布时间、链接和摘要。
- 中文技术社区：掘金登录/受限视图识别。
- PDF/论文：arXiv PDF 优先读取摘要页；本地文本型 PDF 可用轻量 `pypdf` 读取，扫描件降级提示。
- 视频字幕：YouTube、B站。
...
- 能读 raw 就不读网页外壳。
- GitHub repo 默认只读 README；只有 `--read-depth full` 才读取 README、docs 和根目录 manifest。
- 能读字幕就不处理音视频本体。
- 能读 feed 条目就不抓完整网页外壳。
- 能读摘要页就不直接读取整篇 PDF。
- 能读主帖和少量高价值评论，就不拉完整讨论串。
- 默认先走 `fast`，只有登录墙或 JS 空壳才用 `browser/auto`。
- Jina Reader 只作为显式 action 兜底，不默认把 URL 交给外部服务。
...
| `--action <id>` | 执行 action（`continue_deep_read` / `extract_outline` / `extract_code` / `login_with_browser` / `read_with_jina`） |
...
- `jina`：内部模式，只由 `read_with_jina` action 触发；通过 Jina Reader 外部服务读取公开网页。
...
pypdf 默认**不装**。读取本地文本型 PDF 时，如果输出提示缺失，再按需安装到项目本地 `.source-reader/vendor`：
...
- `read_with_jina` (`reader`)：当公开网页读取结果为空、被阻断或疑似 JS 空壳时出现；显式通过 Jina Reader 外部服务重试。
- `install_pdf_reader` (`reader`)：当本地 PDF 缺少 `pypdf` 时出现，安装轻量 PDF 文本读取依赖。
- `online_pdf_parse_explicit_upload` (`reader`)：当 PDF 是扫描件或复杂版式时出现，只表示用户可显式选择在线模型解析，不会自动上传。
- `install_local_whisper_heavy` (`reader`)：当视频无字幕且没有可用转写能力时出现，安装本地 Whisper 重型兜底。
...
python3 scripts/source_reader.py read <source> --action read_with_jina --format md
```
...
`status` 会同时汇总 backend capabilities 和最近 failure log，按 domain 和 failure type 给出建议，例如登录态过期、JS 空壳、视频依赖缺失、PDF 无文本。
...
`status` / `--doctor` 还会输出 backend capabilities，用来确认当前机器能读哪些输入类型：本地文件、GitHub、RSS/feed、普通网页、browser/scrapling 网页、PDF、视频字幕和显式 Jina fallback。
...
pypdf 是本地 PDF 文本读取的轻量可选依赖；只影响本地文本型 PDF，不影响网页、GitHub、browser 模式：
